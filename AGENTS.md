...
- 模块职责固定：`main.js` 只做装配；`api.js` 负责请求与错误解析；`stars.js`（星空与流星）/ `bubbles.js`（亮色泡泡）/ `cursor.js`（光标粒子）/ `theme.js` / `projects.js` / `guestbook.js` / `ui.js` 各管一块。模块之间只通过 `export` 通信，禁止跨模块查询对方 DOM。
